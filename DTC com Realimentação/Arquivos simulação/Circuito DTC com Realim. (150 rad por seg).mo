model controle_para100
  ...
  Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage imc(Jr(start = 0.1170), Js = 0.1170, Lm = 0.1635, Lrsigma = 0.324e-3, Lssigma = 0.324e-3, Lszero = 0.324e-3, Rr = 0.04, Rs = 0.03, TrOperational(displayUnit = "K") = 293.15, TrRef(displayUnit = "K") = 293.15, TsOperational(displayUnit = "K") = 293.15, TsRef(displayUnit = "K") = 293.15, alpha20r = 0, alpha20s = 0, fsNominal = 50, p = 2, phiMechanical(displayUnit = "rad"), useSupport = false, wMechanical(displayUnit = "rad/s"))  annotation(
  ...
  Modelica.Blocks.Sources.RealExpression realExpression1(y = 150)  annotation(
    Placement(visible = true, transformation(origin = {-256, -86}, extent = {{-14, -16}, {14, 16}}, rotation = 0)));
  ...
  Modelica.Electrical.Polyphase.Sensors.CurrentSensor currentSensor annotation(
    Placement(visible = true, transformation(origin = {48, 96}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  ...
  Modelica.Mechanics.Rotational.Sources.QuadraticSpeedDependentTorque quadraticSpeedDependentTorque(tau_nominal = -161.4, useSupport = true, w_nominal = 157)  annotation(
    Placement(visible = true, transformation(origin = {140, 84}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Continuous.PI pi(T = 0.02, k = 0.7)  annotation(
    Placement(visible = true, transformation(origin = {-10, -92}, extent = {{-16, -16}, {16, 16}}, rotation = 0)));
  Modelica.Blocks.Math.Feedback feedback annotation(
    Placement(visible = true, transformation(origin = {-153, -85}, extent = {{19, -19}, {-19, 19}}, rotation = 180)));
equation
  ...
  connect(inductor.plug_n, currentSensor.plug_p) annotation(
    Line(points = {{14, 120}, {48, 120}, {48, 106}}, color = {0, 0, 255}));
  connect(currentSensor.plug_n, imc.plug_sp) annotation(
    Line(points = {{48, 86}, {48, 56}}, color = {0, 0, 255}));
  ...
  connect(currentSensor.i, fluxo_e_Torque.I) annotation(
    Line(points = {{37, 96}, {-52, 96}, {-52, -44}, {-36, -44}}, color = {0, 0, 127}, thickness = 0.5));
  connect(quadraticSpeedDependentTorque.support, fixed1.flange) annotation(
    Line(points = {{140, 74}, {140, 50}}));
  connect(imc.flange, quadraticSpeedDependentTorque.flange) annotation(
    Line(points = {{52, 46}, {104, 46}, {104, 84}, {130, 84}}));
  connect(imc.flange, speedSensor.flange) annotation(
    Line(points = {{52, 46}, {82, 46}, {82, 36}}));
  connect(speedSensor.w, feedback.u2) annotation(
    Line(points = {{82, 16}, {82, 2}, {-152, 2}, {-152, -70}}, color = {0, 0, 127}));
  connect(realExpression1.y, feedback.u1) annotation(
    Line(points = {{-240, -86}, {-205, -86}, {-205, -84}, {-168, -84}}, color = {0, 0, 127}));
  connect(feedback.y, pi.u) annotation(
    Line(points = {{-136, -84}, {-108, -84}, {-108, -92}, {-29, -92}}, color = {0, 0, 127}));
  connect(pi.y, histerese_3.Tr) annotation(
    Line(points = {{8, -92}, {50, -92}, {50, -68}, {142, -68}}, color = {0, 0, 127}));
  annotation(
